    ...
    <Section id="experience" subtitle="Professional History" title="Work" titleAccent="Experience">
      <div className="space-y-0">
        {cvData.experience.map((exp, i) => (
          <motion.div
            key={i}
            className="border-t border-primary/10 py-10"
          ...
          >
            <div className="grid md:grid-cols-[220px_1fr] gap-4 md:gap-16">
              ...
              <div>
                <h3 className="text-xl md:text-2xl font-serif text-primary mb-4">{exp.role}</h3>
                <ul className="space-y-2.5 text-primary/65">
                  {exp.highlights.map((h, j) => (
                    <li key={j} className="flex items-start gap-2.5 leading-relaxed text-[15px]">
                ...
                {i === cvData.experience.length - 1 && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center mt-4 text-sm text-[hsl(40,45%,55%)] hover:text-primary transition-colors"
                  >
                    View Profile →
                  </a>
                )}
